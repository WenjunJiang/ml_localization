...
@LastEditTime: 2020-05-31 23:48:34
@Description: 
'''
import os
...
def get_batch(X,Y,batch_size):
    ...
    new_input = np.zeros((batch_size,9))
    ...
    for i in range(batch_size):
        if i < batch_size//2:
            ...
            new_input[i,:]=X_train_0[index]
        else:
            index = rng.randint(0,len_train_1)
            new_input[i,:]=X_train_1[index]
...
if __name__=='__main__':
    ...
    for i in range(iteration):
        (inputs, labels) = get_batch(X_train, Y_train, batch_size)
        ...
        if i % record_every == 0:
            ...
            if val_acc >= best:
                ...
                best = val_acc
        if i % evaluate_every_2 ==0:
            test_task(model, X_test, Y_test, mode=1)
    
    val_acc = test_task(model, X_test, Y_test)
    test_task(model, X_test, Y_test, mode=1)

    plt.plot(range(iteration), losses, 'bo', label='Training loss')
    plt.show()